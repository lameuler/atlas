...
async function runCallback<T extends unknown[], R>(
    ...
    try {
        ...
        throw new PageError(info.dest, `failed to run \`${name}\`${message}`, {
            cause: err,
            src: info.src,
        })
    }
...
export async function processPage(
    ...
    try {
        ...
        const url = page.url()
        const dest =
            baseUrl && url.startsWith(baseUrl?.origin) ? url.substring(baseUrl?.origin.length) : url
        ...
        if (outPath.endsWith(sep) || outPath.endsWith('/')) {
            throw new PageError(dest, `output path \`${outPath}\` is a directory`, {
                src: location,
            })
        }
            ...
            if (!context.closed) {
                ...
                try {
                    ...
                    debug(
                        bold(
                            red(
                                `failed to close browser context (${context.id}) for \`${location}\`: `,
                            ),
                        ) + err,
                    )
                }
                ...
                debug(
                    yellow(
                        `browser context (${context.id}) for \`${location}\` has already been closed`,
                    ),
                )
            }
        }
